...
{
    [HarmonyPatch]
    public static class ZombieBearWallCleavePatch
    {
        // ===== ZOMBIE BEAR ONLY =====
        // Mirrors current successful Grace wall-cleave profile,
        // but applies ONLY to animalZombieBear.

        private const float ZombieBearDirectMultiplier = 5f;

        private const float ZombieBearSideFrac = 2f;
        private const float ZombieBearUpFrac = 0.50f;
        private const float ZombieBearDownFrac = 0.35f;

        private const int ZombieBearFallbackBaseDamage = 300;
        ...
        public static void CalculateBlockDamage_Postfix(EntityAlive __instance, BlockDamage block, int defaultBlockDamage, ref int __result)
        {
            try
            {
                ...
                if (!IsZombieBear(__instance)) return;

                int boosted = Mathf.RoundToInt(__result * ZombieBearDirectMultiplier);
                if (boosted > __result)
                    __result = boosted;
            ...
            {
                Debug.Log($"[DeadAir][ZombieBearWallCleave] CalculateBlockDamage_Postfix error: {ex}");
            }
        }
        ...
        public static void ExecuteDestroyBlockBehavior_Postfix(EntityAlive __instance, object behavior, object attackHitInfo, ref bool __result)
        {
            try
            {
                ...
                ApplySplash(world, pattern.Left,  ClampSplash(mainDamage, ZombieBearSideFrac), __instance.entityId);
                ApplySplash(world, pattern.Right, ClampSplash(mainDamage, ZombieBearSideFrac), __instance.entityId);
                ApplySplash(world, pattern.Up,    ClampSplash(mainDamage, ZombieBearUpFrac),   __instance.entityId);
                ApplySplash(world, pattern.Down,  ClampSplash(mainDamage, ZombieBearDownFrac), __instance.entityId);

                if (DamageCenterForward && pattern.Forward != hitPos)
                    ApplySplash(world, pattern.Forward, ClampSplash(mainDamage, ZombieBearSideFrac * 0.85f), __instance.entityId);
            }
            catch (Exception ex)
            {
                Debug.Log($"[DeadAir][ZombieBearWallCleave] ExecuteDestroyBlockBehavior_Postfix error: {ex}");
            }
        }

        private static bool IsZombieBear(EntityAlive entity)
        {
            if (entity == null) return false;

            string runtimeName = entity.GetType().Name.ToLowerInvariant();
            if (runtimeName.Contains("zombiebear"))
                return true;

            string className = GetEntityClassName(entity).ToLowerInvariant();
            return className == "animalzombiebear";
        }
        ...
        private static int GetMainDamage(EntityAlive entity)
        {
            ...
            int baseDmg = TryGetIntMethod(entity, "GetBlockDamage") ?? ZombieBearFallbackBaseDamage;
            return Mathf.RoundToInt(baseDmg * ZombieBearDirectMultiplier);
        }
